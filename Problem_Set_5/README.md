...
![](HW5_files/figure-markdown_strict/unnamed-chunk-2-1.png)
...
### b).
    ...
    ## [1] 0.013 0.999 0.993 0.999 0.998 0.998 0.999

    hist(post.samples$samples[,1], main = "a")
    abline(v = 4, lwd=3)

![](HW5_files/figure-markdown_strict/unnamed-chunk-4-1.png)

    hist(post.samples$samples[,2], main = "s")
    abline(v = 3, lwd=3)

![](HW5_files/figure-markdown_strict/unnamed-chunk-4-2.png)

    hist(post.samples$samples[,3], main = "m")
    abline(v = 2, lwd=3)

![](HW5_files/figure-markdown_strict/unnamed-chunk-4-3.png)
...
### b). Compute the BIC for each fit and record whether k = 4, 5, or 6 had the lowest BIC.
    ...
    ## Warning in sqrt(sum(t[, i] * (y^2))/sum(t[, i]) - (mu[i])^2): NaNs produced
...
Since the BIC picks k = 4, there is a little uncertainty and we should
not ignore uncertainty when estimate parameters.
